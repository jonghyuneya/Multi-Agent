...
async def process_user_question(user_question: str, is_clarification: bool = False):
    ...
    try:
        ...
        if isinstance(agent_state, dict):
            print(f"Agent state before processing: user_question='{agent_state.get('user_question', '')}', user_clarification='{agent_state.get('user_clarification', '')}', clarification_confirmed={agent_state.get('clarification_confirmed', False)}")
        else:
