...
@tool
async def question_clarity_checker(user_question: str) -> str:
    """Check if the user question is clear and specific enough for processing."""
    try:
        prompt = f"""
        You are a helpful assistant that evaluates if user questions are clear enough to process.
        
        IMPORTANT: Be lenient and only flag questions that are truly unclear or ambiguous.
        Questions like "What is the weather today?", "Tell me about AI", "How does this work?" are CLEAR.
        Only flag questions that are extremely vague like single words or very unclear references.
        ...
        Clarification: [if unclear, suggest what to ask the user]
        """
        response = await llm.ainvoke([HumanMessage(content=prompt)])
        result = response.content
        print(f"Question clarity check: {result}")
        return result
    except Exception as e:
        print(f"Error checking question clarity: {str(e)}")
        return "Clear: YES\nReason: Default to clear"
...
# Define tools list
tools = [naver_api, typo_service, summarizer, intent_classifier, question_clarity_checker]
