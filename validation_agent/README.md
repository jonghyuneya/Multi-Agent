...
### 1. 투자 브리핑 스크립트 (Investment Briefing)

SEC 공시, 뉴스 기사, 시장 데이터를 출처로 사용하는 투자 분석 스크립트:
...
```json
{
    ...
    "rounds": [
        {
            "round": 1,
            "fundamental": "영업마진 30.5%를 유지하며... (2025-10-30 제출 10-Q 기준)",
            "risk": "AI CAPEX 증가로 단기 마진 압박 가능성",
            "growth": "Google Cloud 성장률 YoY 25% 이상",
            "sentiment": "뉴스 ID 8에서 AI 경쟁력 회복 평가"
        }
    ],
    ...
    "sources": {
        "ticker": "GOOG",
        "sec_filings": [
            {"form": "10-Q", "filed_date": "2025-10-30", "accession_number": "..."}
        ],
        "news_articles": [
            {"id": 8, "title": "Google started the year behind...", "source": "Yahoo Finance"}
        ],
        "market_data": {"current_price": 314.96, "pe_ratio": 31.06}
    }
}
```
...
```python
from validation_agent import validate_investment_briefing

with open("briefing_result.json") as f:
    script_json = f.read()

result = validate_investment_briefing(script_json, validate_content=True)
print(result.summary)
```

### 2. 텍스트 스크립트 (closing_briefing 출력)
...
### 구조화된 브리핑 스크립트 (closing_briefing)
...
### 투자 브리핑 스크립트 (Investment Briefing)

| 유형 | 설명 | 식별자 |
|------|------|--------|
| `sec_filings` | SEC 공시 | `form`, `filed_date`, `accession_number` |
| `news_articles` | 뉴스 기사 | `id`, `title` |
| `market_data` | 시장 데이터 | `current_price`, `pe_ratio`, `market_cap` |

**투자 브리핑 검증 방법:**

1. **인용 추출**: 스크립트 텍스트에서 "뉴스 ID 8", "2025-10-30 제출 10-Q" 등의 패턴을 찾음
2. **출처 확인**: 추출된 인용이 `sources` 필드에 실제로 존재하는지 확인
3. **내용 검증**: LLM을 사용하여 스크립트 내용이 출처와 일치하는지 심층 검증
...
### Yahoo Finance 뉴스 (DynamoDB)

AWS DynamoDB에서 Yahoo Finance 뉴스를 로드합니다:

```python
# DynamoDB 테이블: kubig-YahoofinanceNews
# 스키마: pk, title, url, provider, publish_et_iso, tickers, path

from validation_agent import YahooFinanceSourceTool

tool = YahooFinanceSourceTool(
    table_name="kubig-YahoofinanceNews",
...
# 로컬 JSON 폴백
python -m validation_agent.main --script script.txt \
    --yahoo-finance /path/to/yahoo_news.json
```
...
```python
from validation_agent import LiveMarketDataSourceTool

tool = LiveMarketDataSourceTool(
    data_path=Path("/path/to/market_data.json")
)
```
...
```json
{
  "^GSPC": {"close": 6050.23, "change_pct": 0.85, "name": "S&P 500"},
  "AAPL": {"close": 255.50, "change_pct": 1.2, "volume": 50000000}
}
```
...
SEC 제출 서류 (10-K, 10-Q, 8-K 등):

```python
from validation_agent import SECEdgarSourceTool

tool = SECEdgarSourceTool(
    data_path=Path("/path/to/sec_filings/")
)
```
